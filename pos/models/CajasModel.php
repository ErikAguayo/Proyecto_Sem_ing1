...
class CajasModel extends Query
{
    ...
    {
        $sql = "SELECT SUM($campo) AS total FROM ventas WHERE metodo = 'CONTADO' AND apertura = 1 AND id_usuario = $id_usuario";
        return $this->select($sql);
    }
    ...
    {
        $sql = "SELECT SUM(total) AS total FROM compras WHERE apertura = 1 AND id_usuario = $id_usuario";
        return $this->select($sql);
    }
